...
class EqualityDefinition {
  ...
  public:
    explicit EqualityDefinition(const string& alphabet,
        ...
        if (additionalEqualities != nullptr) {
     ...
     */
    [[nodiscard]] bool areEqual(unsigned char a, unsigned char b) const {
        return matrix[a][b];
    }
};
...
static inline Word* buildPeq(int alphabetLength,
...
extern "C" EdlibAlignConfig edlibDefaultAlignConfig(void) {
    return edlibNewAlignConfig(-1, EDLIB_MODE_NW, EDLIB_TASK_DISTANCE, nullptr, 0);
}
